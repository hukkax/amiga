...
;	D6 = Line Mode (0=normal, 1=area outline)

DL_MInterns_Normal = $CA ; CA = normal
DL_MInterns_Area   = $4A ; 4A = xor
...
.xdpos:	moveq	#$F, d4			; D4 full cleaned (for later oktants move.b)
	...
	ifnz.b	d6, .area1
.norm1	or.w	#$B00+DL_MInterns_Normal, d4	; BLTCON0-codes
	bra	.go
.area1	or.w	#$B00+DL_MInterns_Area, d4	; BLTCON0-codes
...
.dygdx:	add.w	d2, d2			; D2 = xd*2
	...
	add.w	#$42, d3
	lea	BLTAPTL-2(a6), a1	; A1 = CUSTOM+$52

	tst.b	(a6)			; Agnus bug
.wb:	btst	d1, (a6)		; Waiting for the Blitter...
	bne.s	.wb

	ifnz.b	d6, .area2		; Line mode?

	; normal line mode
	bchg	#1, d4

	; area fill mode
.area2	bchg	d5, (a0)		; Inverting the First Bit of Line

	; Writing to the Blitter Regs as fast as possible
	move.l	d4, BLTCON0-2(a6)	; Blitter control register 0
	move.l	d2, BLTBMOD-2(a6)	; Blitter modulo B (modulo for blitter source)
	move.l	a0, BLTCPTH-2(a6)	; Blitter pointer to x (high 3 bits)
...
.wb:	btst	d2, (a6)
	bne.s	.wb
	move.w	d1, BLTAFWM-2(a6)	; Blitter first-word mask for source A
	move.w	d1, BLTBDAT-2(a6)	; Blitter source B data register
	move.w	#$8000, BLTADAT-2(a6)	; Blitter source A data register
	move.w	d0, BLTCMOD-2(a6)	; Blitter modulo C (modulo for blitter source)
	move.w	d0, BLTDMOD-2(a6)	; Blitter modulo D (modulo for blitter source)
	rts
